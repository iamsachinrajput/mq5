...
input ENUM_LOT_CALC_METHOD LotCalc_Boundary = LOT_CALC_BASE;          // Case 1: Boundary orders (topmost BUY, bottommost SELL)
...
input ENUM_LOT_CALC_METHOD LotCalc_Counter = LOT_CALC_BASE;           // Case 3: Counter-direction orders (against movement)
input ENUM_LOT_CALC_METHOD LotCalc_GPO_More = LOT_CALC_BASE;          // Case 4: When GPO > GLO (more profit than loss)
input ENUM_LOT_CALC_METHOD LotCalc_GLO_More = LOT_CALC_GLO;           // Case 5: When GLO > GPO (more loss than profit)
input ENUM_LOT_CALC_METHOD LotCalc_Centered = LOT_CALC_BASE;          // Case 6: Centered position (balanced buy/sell distribution)
input ENUM_LOT_CALC_METHOD LotCalc_Sided = LOT_CALC_GLO;              // Case 7: Sided position (unbalanced distribution)
...
input int FarAdjacentDistance = 1;       // Starting distance for FAR_ADJACENT (1=adjacent, 3=skip 1 level, 5=skip 2 levels)
input int FarAdjacentDepth = 1;          // Number of opposite-type levels to check from starting distance (1=strict, higher=flexible)
...
input int    VlineOffsetCandles = 3;   // Current profit vline offset in candles from current time
...
input ENUM_CTRL_CORNER CtrlButtonCorner = CTRL_CORNER_RIGHT_LOWER; // Control buttons corner position
input int    CtrlBtnXDistance = 10;      // Control buttons X distance from edge
...
input int    CtrlPanelXDistance = 110;   // Panel X distance from edge
input int    CtrlPanelYDistance = 50;    // Panel Y distance from edge
...
int  g_historyDisplayMode = 3;   // History display mode: 0=Overall, 1=CurSymAllMagic, 2=CurSymCurMagic, 3=PerSymbol
...
bool g_showMainButtons = true;   // Show/hide main control buttons (top-right)
...
void CreateButtons() {
   ...
   // Button 11: Total Trail Mode (TRAIL)
   ...
   // Button 12: Debug Level (LESS CRITICAL)
   ...
   // Button 13: Print Stats (LESS CRITICAL)
   ...
   if(ObjectFind(0, btn6Name) < 0) {
      ...
      ObjectSetInteger(0, btn6Name, OBJPROP_YDISTANCE, topMargin + (buttonHeight + verticalGap) * 13);
   ...
   if(ObjectFind(0, btnVLinesName) < 0) {
      ...
      ObjectSetInteger(0, btnVLinesName, OBJPROP_BGCOLOR, clrDarkGreen);
      ObjectSetInteger(0, btnVLinesName, OBJPROP_COLOR, clrWhite);
   ...
   if(ObjectFind(0, btnHLinesName) < 0) {
      ...
      ObjectSetInteger(0, btnHLinesName, OBJPROP_BGCOLOR, clrDarkGreen);
      ObjectSetInteger(0, btnHLinesName, OBJPROP_COLOR, clrWhite);
...
void UpdateButtonStates() {
   ...
   ObjectSetInteger(0, btnVLines, OBJPROP_STATE, false);
   ...
   ObjectSetInteger(0, btnHLines, OBJPROP_STATE, false);
...
void ApplyVisibilitySettings() {
   ...
   string mainButtons[] = {
      ...
      "BtnDebugLevel", "BtnPrintStats", "BtnToggleVLines", "BtnToggleHLines"
   };
...
void CreateCloseOrderLabel(ulong ticket, int level, int orderType, double profit, double price, datetime time) {
   ...
   string labelText = StringFormat("L%d %s %s%.1f", level, typeStr, profit >= 0 ? "+" : "", profit);
...
void ResetAllCounters() {
   ...
   // Reset origin price to current price (level index starts from 0)
   ...
   for(int i = 0; i < totalDeals; i++) {
      ...
      for(int d = 0; d < 5; d++) {
         if(dealDay == dayStarts[d]) {
            ...
            // Track per-symbol profits (for matching magic numbers)
            if(dealMagic == Magic) {
...
void UpdatePositionStats() {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      // Count orders in loss and profit
      if(profit < 0) {
         g_orders_in_loss++;
      } else if(profit > 0) {
         g_orders_in_profit++;
...
//============================= LOT CALCULATION ====================//
// Helper: Apply lot calculation method
double ApplyLotMethod(ENUM_LOT_CALC_METHOD method) {
   ...
   // Count orders in loss and profit by type
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ulong ticket = PositionGetTicket(i);
      if(!PositionSelectByTicket(ticket)) continue;
      if(PositionGetString(POSITION_SYMBOL) != _Symbol) continue;
...
// Helper: Detect all matching lot scenarios for specific order type and level
void DetectAllMatchingScenarios(ENUM_POSITION_TYPE orderType, int level, bool &matchingScenarios[]) {
   ...
   // Check Scenario 1: Boundary Orders (Case 1)
   if((orderType == POSITION_TYPE_BUY && !hasBuyAbove) || 
      (orderType == POSITION_TYPE_SELL && !hasSellBelow)) {
      matchingScenarios[0] = true;
   }
   
   // Check Scenario 2 & 3: Direction vs Counter-direction
   if(sellLossCount > buyLossCount) {
      // Movement is UP (sells losing)
      if(orderType == POSITION_TYPE_BUY) matchingScenarios[1] = true; // Direction (Case 2)
      else matchingScenarios[2] = true; // Counter-direction (Case 3)
   } else if(buyLossCount > sellLossCount) {
      // Movement is DOWN (buys losing)
      if(orderType == POSITION_TYPE_SELL) matchingScenarios[1] = true; // Direction (Case 2)
      else matchingScenarios[2] = true; // Counter-direction (Case 3)
   }
   
   // Check Scenario 4: GPO > GLO (Case 4)
   if(totalGPO > totalGLO) {
      matchingScenarios[3] = true;
   }
   
   // Check Scenario 5: GLO > GPO (Case 5)
   if(totalGLO > totalGPO) {
      matchingScenarios[4] = true;
   }
   
   // Check Scenario 6: Centered Position (Case 6)
   if(countDiff <= CenteredThreshold) {
      matchingScenarios[5] = true;
   }
   
   // Check Scenario 7: Sided Position (Case 7)
   if(countDiff > CenteredThreshold) {
      matchingScenarios[6] = true;
   }
}

// Helper: Detect lot scenario based on priority sequence
int DetectLotScenario(ENUM_POSITION_TYPE orderType, int level) {
   ...
   // Find first matching scenario based on priority sequence
   for(int p = 0; p < 7; p++) {
      int scenarioIndex = g_lotCalcPriority[p] - 1; // Convert 1-based to 0-based
      if(scenarioIndex >= 0 && scenarioIndex < 7 && matchingScenarios[scenarioIndex]) {
         // Check if this scenario's method is not IGNORE
   ...
   // Default: Return first non-ignored scenario, or scenario 5 (centered) if all ignored
   for(int i = 0; i < 7; i++) {
   ...
   return 5; // Fallback to centered
}

// Helper: Get lot method for scenario
...
void CalculateNextLots() {
   // Calculate lots using scenario-based method
   // For simplicity, we'll use current price level as reference
   double currentPrice = (SymbolInfoDouble(_Symbol, SYMBOL_ASK) + SymbolInfoDouble(_Symbol, SYMBOL_BID)) / 2.0;
   int currentLevel = PriceLevelIndex(currentPrice, g_adaptiveGap);
   
   // Detect scenario and calculate lot for BUY
   ...
   // Detect scenario and calculate lot for SELL
   ...
   string scenarioNames[] = {"Boundary", "Direction", "Counter", "GPO>GLO", "GLO>GPO", "Centered", "Sided"};
   ...
   // Override with calculated lot size if trail is active and mode requires it
   if(g_trailActive && (TrailOrderMode == TRAIL_ORDERS_BASESIZE || 
      ...
      // Calculate lot size based on TrailLotMode
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
   if(id == CHARTEVENT_OBJECT_CLICK) {
      ...
      if(StringFind(sparam, "SelectLabel_") >= 0 && sparam != "SelectLabel_ALL") {
         ...
         Log(2, StringFormat("Label %d visibility toggled: %s", selectedLabel, isVisible ? "VISIBLE" : "HIDDEN"));
      }
...
// Parse priority sequence from input string
void ParsePrioritySequence() {
   // Default sequence: 1,2,3,4,5,6,7
   for(int i = 0; i < 7; i++) {
   ...
   if(count > 0 && count <= 7) {
      ...
      // Fill remaining with default sequence (any missing numbers)
      if(validCount < 7) {
...
void UpdateCurrentProfitVline() {
   ...
   if(g_showLabels) {
      ...
      if(g_showNextLotCalcLabel) {
         string scenarioNames[] = {"Boundary", "Direction", "Counter", "GPO>GLO", "GLO>GPO", "Centered", "Sided"};
         string buyScenarioName = (g_nextBuyScenario >= 0 && g_nextBuyScenario < 7) ? scenarioNames[g_nextBuyScenario] : "Unknown";
         string sellScenarioName = (g_nextSellScenario >= 0 && g_nextSellScenario < 7) ? scenarioNames[g_nextSellScenario] : "Unknown";
         
         string nextLotText = StringFormat("NextLots: B[%s|%s|%.2f] S[%s|%s|%.2f] GLO:%d GPO:%d",
               buyScenarioName, g_nextBuyMethod, g_nextBuyLot,
               sellScenarioName, g_nextSellMethod, g_nextSellLot,
               g_orders_in_loss, g_orders_in_profit);
         color nextLotColor = clrLightSkyBlue;
         UpdateOrCreateLabel("NextLotCalcLabel", 10, 90, nextLotText, nextLotColor, 9, "Arial Bold");
      } else {
      ...
      if(g_showSingleTrailLabel) {
         ...
         UpdateOrCreateLabel("SingleTrailStatusLabel", 10, 110, singleTrailStatus, singleTrailColor, 11, "Arial Bold");
      } else {
      ...
      if(g_showGroupTrailLabel) {
         ...
         UpdateOrCreateLabel("GroupTrailStatusLabel", 10, 130, groupTrailStatus, groupTrailColor, 11, "Arial Bold");
      } else {
      ...
      if(g_showTotalTrailLabel) {
         ...
         UpdateOrCreateLabel("TotalTrailStatusLabel", 10, 150, totalTrailStatus, totalTrailColor, 11, "Arial Bold");
      } else {
      ...
      string sellInfo = (nearestSellLevel != -999999) ? StringFormat("%dS%.2f[%s]", nearestSellLevel, nearestSellLot, sellComment) : "-";
         ...
         UpdateOrCreateLabel("LevelInfoLabel", 10, 170, label4text, label4Color, 10, "Arial Bold");
      } else {
      ...
      if(g_showNearbyOrdersLabel) {
         ...
         UpdateOrCreateLabel("NearbyOrdersLabel", 10, 190, label5text, label5Color, 10, "Arial Bold");
      } else {
      ...
      if(g_showLast5ClosesLabel) {
         ...
         UpdateOrCreateLabel("Last5ClosesLabel", 10, 210, label6text, clrYellow, 9, "Arial");
      } else {
      ...
      UpdateOrCreateLabel("HistoryDisplayLabel", 10, 228, historyLabel, historyColor, 9, "Arial Bold");
      } else {
