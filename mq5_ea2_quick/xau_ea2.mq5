...
// Helper: Detect lot scenario for specific order type and level
int DetectLotScenario(ENUM_POSITION_TYPE orderType, int level) {
   ...
   // Scenario 1: Boundary Orders
   if(orderType == POSITION_TYPE_BUY && !hasBuyAbove) return 0; // Top BUY
   if(orderType == POSITION_TYPE_SELL && !hasSellBelow) return 0; // Bottom SELL
   
   // Scenario 6: Centered Position (small buy/sell difference)
   if(countDiff <= CenteredThreshold) return 5;
   
   // Scenario 7: Sided Position (imbalanced but not boundary)
   if(countDiff > CenteredThreshold) return 6;
   
   // Scenario 4: GPO > GLO
   if(totalGPO > totalGLO) return 3;
   
   // Scenario 5: GLO > GPO
   if(totalGLO > totalGPO) return 4;
   
   // Scenario 2 & 3: Direction vs Counter-direction
   // Direction order = opposite to accumulation side
   // If more sells in loss, movement is UP, direction order is BUY
   // If more buys in loss, movement is DOWN, direction order is SELL
   if(sellLossCount > buyLossCount) {
      // Movement is UP (sells losing)
      if(orderType == POSITION_TYPE_BUY) return 1; // Direction
      else return 2; // Counter-direction
   } else if(buyLossCount > sellLossCount) {
      // Movement is DOWN (buys losing)
      if(orderType == POSITION_TYPE_SELL) return 1; // Direction
      else return 2; // Counter-direction
   }
...
void CalculateNextLots() {
   ...
   g_nextBuyLot = NormalizeLotSize(ApplyLotMethod(buyMethod));
   ...
   g_nextSellLot = NormalizeLotSize(ApplyLotMethod(sellMethod));
   ...
   Log(3, StringFormat("Lot Calc: BUY Scenario=%s Method=%s Lot=%.2f | SELL Scenario=%s Method=%s Lot=%.2f",
       scenarioNames[buyScenario], methodNames[buyMethod], g_nextBuyLot,
       scenarioNames[sellScenario], methodNames[sellMethod], g_nextSellLot));
...
string GetSingleTrailStatusInfo() {
   ...
   // Count active single trails
   int activeTrails = 0;
   double totalProfit = 0.0;
   for(int i = 0; i < ArraySize(g_trails); i++) {
      if(g_trails[i].active) {
         activeTrails++;
         // Find position to get profit
...
void UpdateCurrentProfitVline() {
   ...
   if(g_showLabels) {
      ...
      UpdateOrCreateLabel("SingleTrailStatusLabel", 10, 90, singleTrailStatus, singleTrailColor, 11, "Arial Bold");
      ...
      UpdateOrCreateLabel("GroupTrailStatusLabel", 10, 110, groupTrailStatus, groupTrailColor, 11, "Arial Bold");
      ...
      UpdateOrCreateLabel("TotalTrailStatusLabel", 10, 130, totalTrailStatus, totalTrailColor, 11, "Arial Bold");
      ...
      UpdateOrCreateLabel("LevelInfoLabel", 10, 150, label4text, label4Color, 10, "Arial Bold");
      ...
      UpdateOrCreateLabel("NearbyOrdersLabel", 10, 170, label5text, label5Color, 10, "Arial Bold");
      ...
      UpdateOrCreateLabel("Last5ClosesLabel", 10, 190, label6text, clrYellow, 9, "Arial");
      
      // Label 10: Last 5 Days Overall (from history)
      string label7text = "Last5D Overall: ";
      for(int i = 0; i < 5; i++) {
         label7text += StringFormat("%.2f", g_historyOverallDaily[i]);
         if(i < 4) label7text += " | ";
      }
      UpdateOrCreateLabel("Last5DaysOverallLabel", 10, 208, label7text, clrYellow, 9, "Arial");
      
      // Labels 11+: Last 5 Days Per Symbol (from history)
      int yOffset = 226;
      for(int s = 0; s < ArraySize(g_symbolHistory); s++) {
         string symbolLabel = StringFormat("Last5D %s: ", g_symbolHistory[s].symbol);
         for(int i = 0; i < 5; i++) {
            symbolLabel += StringFormat("%.2f", g_symbolHistory[s].daily[i]);
            if(i < 4) symbolLabel += " | ";
         }
         string labelName = StringFormat("Last5DaysSymbol_%s", g_symbolHistory[s].symbol);
         UpdateOrCreateLabel(labelName, 10, yOffset, symbolLabel, clrCyan, 9, "Arial");
         yOffset += 18;
      }
   } else {
   ...
   if(g_showLabels) {
      // Label 10: Center Label Line 1 - Total Profit / GLO
      long chartWidth = ChartGetInteger(0, CHART_WIDTH_IN_PIXELS);
      long chartHeight = ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS);
      ...
      // Label 11: Center Label Line 2 - Current Cycle Profit / Booked Profit in this Cycle
      ...
      // Label 12: Center Label Line 3 - Max Loss Cycle / Max Overall Loss / Max Lot Size
